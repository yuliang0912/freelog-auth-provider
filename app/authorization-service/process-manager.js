...
//      */
//     async presentableAuthTreeAuthorization({presentableAuthTree, userInfo, nodeInfo, userContract}) {
//
//     }
//
//     /**
...
//      * @private
//      */
//     async _authSchemeContractsAuth({presentableAuthTree, nodeInfo}) {
//
//         const unActivatedContracts = [], resourceContracts = [], nodeContracts = []
//         const authSchemeContractIds = presentableAuthTree.authTree.map(x => x.contractId)
//         await this.dataProvider.contractProvider.find({_id: {$in: authSchemeContractIds}}).each(current => {
//             if (current.status !== 3) {
//                 unActivatedContracts.push(current)
//             }
//             if (current.contractType === this.app.contractType.ResourceToNode) {
//                 nodeContracts.push(current)
//             }
//             if (current.contractType === this.app.contractType.ResourceToResource) {
//                 resourceContracts.push(current)
//             }
//         })
//         if (unActivatedContracts.length) {
//             throw Object.assign(new Error('presentable授权树中存在未激活的合同'), {data: unActivatedContracts})
//         }
//
//         nodeContracts.map(nodeContract => {
//             this.nodeContractAuthorization({nodeContract, nodeInfo, userInfo})
//         })
//
//
